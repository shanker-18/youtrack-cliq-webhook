...
def update_dashboard(n_clicks, gbu, squad, model, c_clicks, s_clicks, active_tab_state, loaded_dashboard_state):
    ...
    valid_squads_norm = [database.normalize_text(s["value"]) for s in squad_opts]
    if database.normalize_text(squad) not in valid_squads_norm:
        squad = "Select Need State"
    ...
    valid_models_norm = [database.normalize_text(m["value"]) for m in model_opts]
    if database.normalize_text(model) not in valid_models_norm:
        model = "Select Model"
    ...
    if should_fetch_snowflake:
        ...
        if current_loaded_store and isinstance(current_loaded_store, dict) and "records" in current_loaded_store:
            ...
            df = pd.DataFrame()
            record_str = "Filter Selection (Excel Only - Click REFRESH DATA to fetch Snowflake)"
    ...
    if should_fetch_snowflake and not df.empty:
        data_by_year = {}
        sample_logs = []
        unmapped_dates = []
        ...
        def make_grouped_rows(metric_name, unit, yr_val_tuples, metric_key, year_metrics):
            ...
            for idx, (yr, m_vals) in enumerate(yr_val_tuples):
                ...
                is_grey_highlight = (yr in ["2026", "YoY %"])
                ...
                for m_idx in range(12):
                    ...
                    if metric_key == "build" and yr == "2026":
                        if v is not None and not (isinstance(v, float) and math.isnan(v)):
                            val_float = float(v)
                            hist_builds = []
                            for h_yr in ["2022", "2023", "2024", "2025"]:
                                ...
                                if b_val is not None and not (isinstance(b_val, float) and math.isnan(b_val)):
                                    hist_builds.append(float(b_val))

                            if len(hist_builds) >= 2:
                                h_mean = statistics.mean(hist_builds)
                                h_std = statistics.stdev(hist_builds)
                                two_std = 2.0 * h_std
                                diff = abs(val_float - h_mean)
                                is_red = (diff > two_std)

                                m_name = MONTHS[m_idx] if m_idx < len(MONTHS) else f"Month {m_idx+1}"
                                if m_idx == 0:
                                    print("=" * 70)
                                    print("STD CONDITIONAL FORMATTING FORMULA VERIFICATION")
                                    print("=" * 70)
                                    print("OLD FORMULA: ABS(2026_val - AVERAGE(2021:2025_same_month)) > 2 * STDEV.S(2021:2025_same_month)")
                                    print("NEW FORMULA: ABS(2026_val - AVERAGE(2022:2025_same_month)) > 2 * STDEV.S(2022:2025_same_month)")
                                    print("=" * 70)

                                print(f"2026 BUILD RED CHECK [{m_name}]: 2026 Build={val_float:.4f}, Mean(2022-2025)={h_mean:.4f}, STDEV.S={h_std:.4f}, 2*STDEV.S={two_std:.4f}, Diff={diff:.4f} -> RED: {'YES' if is_red else 'NO'}")
                ...
                ytd_val = calc_period_val(yr, slice(0, 8))
                ytg_val = calc_period_val(yr, slice(8, 12))
        ...
        if active_tab == "shipments":
            ...
            yoy_gts = [calc_pct_var(year_metrics["2026"]["gross_ship"][i], year_metrics["2025"]["gross_ship"][i]) for i in range(12)]
            yoy_gstu = [calc_pct_var(year_metrics["2026"]["gross_case"][i], year_metrics["2025"]["gross_case"][i]) for i in range(12)]
            yoy_b3 = [calc_pct_var(year_metrics["2026"]["net_ship"][i], year_metrics["2025"]["net_ship"][i]) for i in range(12)]
            yoy_build_bleed = [calc_pct_var(year_metrics["2026"]["build"][i], year_metrics["2025"]["build"][i]) for i in range(12)]
            yoy_unit_ratio = [calc_pct_var(year_metrics["2026"]["share"][i], year_metrics["2025"]["share"][i]) for i in range(12)]
            ...
            yoy_pos = [calc_pct_var(year_metrics["2026"]["pos_val"][i], year_metrics["2025"]["pos_val"][i]) for i in range(12)]
            yoy_factory = [calc_pct_var(year_metrics["2026"]["factory_pos"][i], year_metrics["2025"]["factory_pos"][i]) for i in range(12)]
            yoy_u = [calc_pct_var(year_metrics["2026"]["pos_u"][i], year_metrics["2025"]["pos_u"][i]) for i in range(12)]
            yoy_asp = [calc_pct_var(year_metrics["2026"]["asp"][i], year_metrics["2025"]["asp"][i]) for i in range(12)]
            yoy_build = [calc_pct_var(year_metrics["2026"]["build"][i], year_metrics["2025"]["build"][i]) for i in range(12)]
            yoy_share = [calc_pct_var(year_metrics["2026"]["share"][i], year_metrics["2025"]["share"][i]) for i in range(12)]
