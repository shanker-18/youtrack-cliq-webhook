...
PREDICTABLE_EFFICIENT_ACCOUNTABLE = [
    (
        "Predictable",
        "Our top 10 brands require higher accuracy, earlier signal detection and tighter assumption management.",
        [
            "Delivering a reliable, consumption-based, assumption-driven plan that is more accurate for priority brands.",
            "Improve BIAS/MAPE and clearly explains monthly changes."
        ]
    ),
    (
        "Efficient",
        "Reduce waste & Increase Discipline with Directionally Accurate > Precisely Wrong.",
        [
            "Eliminating duplicate work and repeated content across Forums (DR, PMR and MBR)",
            "Simplifying building blocks",
            "Reducing manual effort",
            "Standardizing templates & expectations",
            "Making DR/PMR/MBR decisive, not performative"
        ]
    ),
    (
        "Accountable",
        "Strengthen Long-Term, Cross-Functional Accountability",
        [
            "The top 10 priority brands get deeper scenario planning, stronger BB ownership, and more robust R&Os.",
            "Mid-tier brands follow a streamlined, standardized process",
            "Long-tail brands rely more heavily on baseline+few/select BBs"
        ]
    )
]
...
PURPOSE = {
    "title": "Purpose:",
    "main_points": [
        "The purpose of the IBP application is to define the requirements and process steps for implementing a centralized consumption-based forecasting platform and delivers a single aligned demand plan (\"One Number\") for Kenvue.",
        "This IBP application ensures:"
    ],
    "sub_points": [
        "Standardization of forecasting inputs and calculations",
        "Improved forecast accuracy and bias reduction",
        "Alignment across Brand, Sales, Finance, and Supply",
        "Reduction in manual reconciliation and inefficiencies"
    ],
...
def section_header(title):
    return html.Div(
        style={"display": "flex", "alignItems": "center", "gap": "10px", "marginBottom": "12px", "paddingLeft": "2px"},
        children=[
...
def make_table(title, headers, rows_data, is_bb=False):
    ...
    th_bg = TRUST_GREEN if is_bb else MINT_HEADER_BG
    th_color = "#ffffff" if is_bb else DARK_TEAL
    widths = ["22%", "38%", "40%"] if is_bb else ["30%", "70%"]

    header_ths = [
        html.Th(h, style={"padding": "10px 14px", "fontSize": "0.76rem", "fontWeight": "800", "color": th_color, "textAlign": "left", "width": widths[i], "letterSpacing": "0.03em", "boxSizing": "border-box"})
    ...
    return html.Div([
        ...
        html.Div(
            style={"borderRadius": "8px", "overflow": "hidden", "border": f"1px solid {BORDER}", "boxShadow": "0 2px 6px rgba(0,0,0,0.02)", "width": "100%", "maxWidth": "100%", "boxSizing": "border-box"},
            children=[
                html.Table(
                    style={"width": "100%", "maxWidth": "100%", "tableLayout": "fixed", "borderCollapse": "collapse", "backgroundColor": "#ffffff", "boxSizing": "border-box"},
                    children=[
...
def build_header():
    title_box = html.Div([
        html.H1("Kenvue", style={"fontSize": "1.05rem", "fontWeight": "900", "color": TRUST_GREEN, "margin": "0", "marginRight": "6px"}),
        html.Span("|", style={"color": BORDER, "marginRight": "8px", "fontWeight": "300"}),
        html.Span("Integrated Business Planning", style={"fontSize": "0.8rem", "color": TEXT_MUTED, "fontWeight": "600"})
    ], style={"display": "flex", "alignItems": "center"})

    login_btn = html.Button([html.Span("Login")], id="btn-login-header", style={"backgroundColor": TRUST_GREEN, "color": "#ffffff", "border": "none", "padding": "7px 22px", "borderRadius": "6px", "fontWeight": "800", "fontSize": "0.82rem", "cursor": "pointer", "boxShadow": "0 2px 6px rgba(1, 152, 129, 0.2)"})

    return html.Header(
        style={"height": "64px", "backgroundColor": "#ffffff", "borderBottom": f"1px solid {BORDER}", "padding": "0 24px", "display": "flex", "alignItems": "center", "justifyContent": "space-between", "position": "sticky", "top": "0", "zIndex": "90", "width": "100%", "boxSizing": "border-box"},
...
def build_home_content():
    sec1_cards = [
        html.Div(
            style={"backgroundColor": "rgba(255, 255, 255, 0.12)", "border": "1px solid rgba(255, 255, 255, 0.22)", "borderRadius": "8px", "padding": "16px 20px", "flex": "1 1 240px", "minWidth": "200px", "boxSizing": "border-box"},
            children=[
                html.Div(t, style={"fontSize": "1rem", "fontWeight": "900", "color": "#ffffff", "marginBottom": "4px"}),
                html.Div(d, style={"fontSize": "0.81rem", "fontWeight": "700", "color": "#E6F5F2", "marginBottom": "10px", "lineHeight": "1.4"}),
                html.Ul(style={"margin": "0", "paddingLeft": "18px", "fontSize": "0.78rem", "color": "#FFFFFF", "lineHeight": "1.55"}, children=[html.Li(b) for b in buls])
            ]
        ) for t, d, buls in PREDICTABLE_EFFICIENT_ACCOUNTABLE
    ]

    hero = html.Div(
        style={"background": f"linear-gradient(135deg, {TRUST_GREEN} 0%, {DARK_TEAL} 100%)", "borderRadius": "10px", "padding": "24px 28px", "marginBottom": "24px", "boxShadow": "0 8px 24px -4px rgba(1, 152, 129, 0.22)", "width": "100%", "boxSizing": "border-box"},
        children=[
            html.Div("Integrated Business Planning Framework", style={"fontSize": "0.75rem", "fontWeight": "800", "color": "#E6F5F2", "textTransform": "uppercase", "letterSpacing": "0.08em", "marginBottom": "6px"}),
            html.H2("Predictable. Efficient. Accountable.", style={"fontSize": "1.45rem", "fontWeight": "900", "color": "#ffffff", "margin": "0 0 18px 0"}),
            html.Div(style={"display": "flex", "flexWrap": "wrap", "gap": "16px", "width": "100%", "boxSizing": "border-box"}, children=sec1_cards)
        ]
    )
    ...
    purpose = html.Div([
        ...
        html.Div(
            style={"backgroundColor": "#FFFFFF", "border": f"1px solid {BORDER}", "borderRadius": "8px", "padding": "16px 20px", "width": "100%", "boxSizing": "border-box"},
            children=[
                html.P(PURPOSE["main_points"][0], style={"fontSize": "0.84rem", "color": TEXT_DARK, "lineHeight": "1.6", "marginBottom": "10px"}),
                html.P(PURPOSE["main_points"][1], style={"fontSize": "0.84rem", "fontWeight": "800", "color": DARK_TEAL, "marginBottom": "6px"}),
                html.Ul(style={"margin": "0 0 10px 0", "paddingLeft": "22px", "fontSize": "0.82rem", "color": TEXT_SECONDARY, "lineHeight": "1.6"}, children=[html.Li(sp) for sp in PURPOSE["sub_points"]]),
                html.Div(PURPOSE["conclusion"], style={"fontSize": "0.82rem", "color": DARK_TEAL, "fontWeight": "800", "backgroundColor": LIGHT_MINT, "padding": "8px 14px", "borderRadius": "6px", "boxSizing": "border-box"})
            ]
        )
    ])
    ...
    scope = html.Div([
        ...
        html.Div(
            style={"backgroundColor": "#FFFFFF", "border": f"1px solid {BORDER}", "borderRadius": "8px", "padding": "16px 20px", "width": "100%", "boxSizing": "border-box"},
            children=[
                html.P(SCOPE["text"], style={"fontSize": "0.84rem", "color": TEXT_DARK, "lineHeight": "1.6", "marginBottom": "12px"}),
                html.Div("This IBP application applies to:", style={"fontSize": "0.84rem", "fontWeight": "800", "color": DARK_TEAL, "marginBottom": "6px"}),
                html.Div(style={"display": "flex", "flexWrap": "wrap", "gap": "8px"}, children=[html.Span(t, style={"backgroundColor": LIGHT_MINT, "color": DARK_TEAL, "padding": "4px 12px", "borderRadius": "20px", "fontSize": "0.78rem", "fontWeight": "800"}) for t in SCOPE["applies_to"]])
            ]
        )
    ])

    eq_box = lambda title, eq: html.Div([
        section_header(title),
        html.Div(style={"backgroundColor": LIGHT_MINT, "border": f"1px solid {MINT_BORDER}", "borderLeft": f"5px solid {TRUST_GREEN}", "borderRadius": "6px", "padding": "14px 20px", "fontFamily": "monospace", "fontSize": "0.88rem", "fontWeight": "800", "color": DARK_TEAL, "width": "100%", "wordWrap": "break-word", "wordBreak": "break-word", "whiteSpace": "normal", "boxSizing": "border-box"}, children=[eq])
    ])

    list_box = lambda title, items: html.Div([
        section_header(title),
        html.Div(style={"backgroundColor": "#FFFFFF", "border": f"1px solid {BORDER}", "borderRadius": "8px", "padding": "16px 20px", "width": "100%", "boxSizing": "border-box"}, children=[html.Ul(style={"margin": "0", "paddingLeft": "20px", "fontSize": "0.84rem", "color": TEXT_DARK, "lineHeight": "1.7"}, children=[html.Li(x) for x in items])])
    ])

    future = html.Div([
        section_header("Future State:"),
        html.Div(style={"backgroundColor": "#FFFFFF", "border": f"1px solid {BORDER}", "borderRadius": "8px", "padding": "18px 22px", "width": "100%", "boxSizing": "border-box"}, children=[
            html.Div([html.Strong("Vision: ", style={"color": TRUST_GREEN, "fontWeight": "900"}), html.Span(FUTURE_STATE["vision"], style={"fontSize": "0.86rem", "fontWeight": "700", "color": TEXT_DARK})], style={"marginBottom": "14px"}),
            html.H4("Core Capabilities of the Tool", style={"fontSize": "0.9rem", "fontWeight": "900", "color": DARK_TEAL, "marginBottom": "10px"}),
            html.Div([html.Div(style={"padding": "12px 14px", "backgroundColor": "#FFFFFF", "border": f"1px solid {BORDER}", "borderRadius": "6px", "marginBottom": "8px", "boxSizing": "border-box"}, children=[
                html.Div(ct, style={"fontSize": "0.84rem", "fontWeight": "800", "color": DARK_TEAL}),
                html.Ul(style={"margin": "4px 0 0 0", "paddingLeft": "18px", "fontSize": "0.78rem", "color": TEXT_SECONDARY, "lineHeight": "1.5"}, children=[html.Li(b) for b in cbuls]) if cbuls else []
            ]) for ct, cbuls in FUTURE_STATE["core_capabilities"]])
        ])
    ])

    doc_container = html.Div(
        style={"backgroundColor": "#FFFFFF", "border": f"1px solid {BORDER}", "borderRadius": "10px", "padding": "28px 32px", "width": "100%", "maxWidth": "100%", "boxSizing": "border-box", "boxShadow": "0 4px 20px -2px rgba(1, 152, 129, 0.06)", "overflowX": "hidden"},
        children=[
            hero, purpose, divider(), scope, divider(),
            make_table("Key Terms:", ["Term", "Definition"], KEY_TERMS), divider(),
            make_table("Roles and Responsibilities", ["Role", "Responsibilities"], ROLES_AND_RESPONSIBILITIES), divider(),
            list_box("POS-Based Forecasting", POS_BASED_FORECASTING), divider(),
            eq_box("Consumption:", CONSUMPTION_EQ), divider(),
            eq_box("Shipments:", SHIPMENT_EQ), divider(),
            future, divider(),
            list_box("Value to the Business:", VALUE_TO_BUSINESS), divider(),
            make_table("1) Consumer POS Consumption Building Blocks (Marketing)", ["Building Block Name", "Definition", "Standardized Methodology/Approach"], CONSUMER_POS_BUILDING_BLOCKS, is_bb=True), divider(),
            make_table("2) Retailer POS & Shipment Building Blocks (Sales Strategy)", ["Building Block Name", "Definition", "Standardized Methodology/Approach"], RETAILER_POS_BUILDING_BLOCKS, is_bb=True), divider(),
            make_table("3) Operational/Conversion Shipment Building Blocks (All)", ["Building Block Name", "Definition", "Standardized Methodology/Approach"], OPERATIONAL_CONVERSION_BUILDING_BLOCKS, is_bb=True), divider(),
            make_table("4) Portfolio Changes POS & Shipment Building Blocks (Marketing)", ["Building Block Name", "Definition", "Standardized Methodology/Approach"], PORTFOLIO_CHANGES_BUILDING_BLOCKS, is_bb=True)
        ]
    )
...
app.layout = html.Div(
    ...
    children=[
        ...
        html.Div(
            ...
            children=[build_header(), html.Div(id="content", children=build_home_content(), style={"flex": "1", "width": "100%", "maxWidth": "100%", "boxSizing": "border-box"})]
        )
    ]
)
