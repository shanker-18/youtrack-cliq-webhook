    ...
    try:
        ...
        if not df.empty:
            ...
            if all_years:
                ...
                # Invoke BB.py calculation & update current month to end of year (DEC) POS $, Factory POS $, & POS U
                if is_model_valid:
                    start_m_nbr = min(latest_comp_m_nbr + 1, 12) if (latest_comp_m_nbr >= 0 and latest_comp_m_nbr < 12) else 12
                    if latest_comp_m_nbr < 12:
                        for m_n in range(start_m_nbr, 13):
                            bb_result = BB.calculate_current_month_pos(df, model, target_year=latest_year, target_month_nbr=m_n)
                            if bb_result:
                                m_i = m_n - 1
                                if bb_result.get("calculated_pos_m") is not None:
                                    # Assign Calculated POS $ (in actual dollars)
                                    data_by_year[latest_year]["pos_val"][m_i] = bb_result["calculated_pos_m"] * 1_000_000.0
                                else:
                                    data_by_year[latest_year]["pos_val"][m_i] = None

                                if bb_result.get("factory_pos_m") is not None:
                                    data_by_year[latest_year]["factory_pos"][m_i] = bb_result["factory_pos_m"] * 1_000_000.0
                                else:
                                    data_by_year[latest_year]["factory_pos"][m_i] = None

                                # Calculate POS U = POS $ / ASP
                                if bb_result.get("calculated_asp") is not None and data_by_year[latest_year]["pos_val"][m_i] is not None:
                                    c_asp = bb_result["calculated_asp"]
                                    if c_asp and c_asp != 0:
                                        data_by_year[latest_year]["pos_u"][m_i] = data_by_year[latest_year]["pos_val"][m_i] / c_asp
            else:
                latest_year = ""
                prev_year = ""
                hist_years = []
                            ...
                            if v is not None and not (isinstance(v, float) and math.isnan(v)):
                                ...
                                if len(hist_builds) >= 2:
                                    try:
                                        ...
                                        m_name = MONTHS[m_idx]
                                        print("\n" + "=" * 75)
                                        print(f"CONSUMPTION BUILD COLOR-CODE CHECK | MODEL: '{model}' | YEAR: {latest_year} | MONTH: {m_name}")
                                        print("=" * 75)
                                        print(f"  - Current Build Value ({latest_year})  : {val_float_2d:.2f}")
                                        print(f"  - Ref Years Builds                    : {hist_builds}")
                                        print(f"  - Average (Mean) of Builds            : {h_mean_2d:.2f}")
                                        print(f"  - Standard Deviation (stdev)        : {h_std:.4f}")
                                        print(f"  - 2 * Standard Deviation (2 * std)  : {two_std_2d:.2f}")
                                        print(f"  - ABS(Current Value - Avg)          : {abs_diff_2d:.2f}")
                                        print(f"  - Condition: ABS({abs_diff_2d:.2f}) > 2*STD({two_std_2d:.2f})  =>  {is_red}")
                                        print(f"  - FLAGGED AS RED CELL              : {'YES 🔴' if is_red else 'NO ⚪'}")
                                        print("=" * 75 + "\n")
                                        ...
                                        print(f"[BUILD COLOR-CODE ERROR]: {err}")
