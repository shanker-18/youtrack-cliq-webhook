            ...
            def make_grouped_rows(metric_name, unit, yr_val_tuples, metric_key, year_metrics):
                ...
                def calc_period_val(target_yr, slice_obj):
                    ...
                    if metric_key in ["pos_val", "factory_pos", "pos_u", "gross_ship", "gross_case", "net_ship", "share"]:
                        ...
                        paired_vals = [
                            (ym["pos_val"][i], ym["pos_u"][i])
                            for i in range(12)[slice_obj]
                            if ym.get("pos_val") and ym.get("pos_u")
                            and ym["pos_val"][i] is not None and ym["pos_u"][i] is not None
                            and not (isinstance(ym["pos_val"][i], float) and math.isnan(ym["pos_val"][i]))
                            and not (isinstance(ym["pos_u"][i], float) and math.isnan(ym["pos_u"][i]))
                            and ym["pos_u"][i] != 0
                        ]
                        if not paired_vals:
                            return None
                        pos_v_sum = sum(pv for pv, pu in paired_vals)
                        pos_u_sum = sum(pu for pv, pu in paired_vals)
                        return (pos_v_sum / pos_u_sum) if pos_u_sum != 0 else None
                    else:
                        arr = ym.get(metric_key, [None]*12)[slice_obj]
                        valid_vals = [v for v in arr if v is not None and not (isinstance(v, float) and math.isnan(v))]
                        return sum(valid_vals) if valid_vals else None
