...
def build_header(user_session=None):
    brand_logo = html.Div(
        style={"display": "flex", "alignItems": "center", "gap": "10px"},
        children=[
            html.Span("KENVUE", style={"fontSize": "1.2rem", "fontWeight": "900", "color": "#000000", "letterSpacing": "0.04em"}),
            html.Span("NA IBP Planning", style={"fontSize": "1.05rem", "fontWeight": "800", "color": TEXT_DARK})
        ]
    )

    ms_icon = html.Div(
        style={"display": "grid", "gridTemplateColumns": "1fr 1fr", "gap": "2px", "width": "14px", "height": "14px", "marginRight": "8px"},
        children=[
            html.Div(style={"backgroundColor": "#F25022", "borderRadius": "1px"}),
            html.Div(style={"backgroundColor": "#7FBA00", "borderRadius": "1px"}),
            html.Div(style={"backgroundColor": "#00A4EF", "borderRadius": "1px"}),
            html.Div(style={"backgroundColor": "#FFB900", "borderRadius": "1px"})
        ]
    )

    if user_session and user_session.get("authenticated"):
        right_action = html.Div(
            style={"display": "flex", "alignItems": "center", "gap": "14px"},
            children=[
                html.Div([
                    html.Div(user_session.get("name", "User"), style={"fontSize": "0.84rem", "fontWeight": "800", "color": TEXT_DARK}),
                    html.Div(user_session.get("email", ""), style={"fontSize": "0.74rem", "color": "#64748B"})
                ]),
                html.Button(
                    "Sign Out",
                    id="btn-signout",
                    style={"backgroundColor": "transparent", "border": f"1px solid {BORDER}", "padding": "6px 14px", "borderRadius": "4px", "fontSize": "0.78rem", "fontWeight": "700", "color": TEXT_DARK, "cursor": "pointer"}
                )
            ]
        )
    else:
        right_action = html.Button(
            children=[ms_icon, html.Span("Sign in")],
            id="btn-signin-header",
            style={
                "backgroundColor": "#0067B8", "color": "#ffffff", "border": "none",
                "padding": "7px 18px", "borderRadius": "4px", "fontWeight": "700",
                "fontSize": "0.84rem", "cursor": "pointer", "display": "flex",
                "alignItems": "center", "boxShadow": "0 2px 5px rgba(0, 103, 184, 0.25)"
            }
        )

    return html.Header(
        style={
            "height": "60px", "backgroundColor": "#ffffff", "borderBottom": f"1px solid {BORDER}",
            "padding": "0 32px", "display": "flex", "alignItems": "center",
            "justifyContent": "space-between", "position": "sticky", "top": "0",
            "zIndex": "1000", "width": "100%", "boxSizing": "border-box"
        },
        children=[brand_logo, right_action]
    )
...
def build_header():
    ...
    return html.Header(
        style={
            "height": "64px", "backgroundColor": "#ffffff", "borderBottom": f"1px solid {BORDER}",
            "padding": "0 32px", "display": "flex", "alignItems": "center",
            "justifyContent": "space-between", "position": "sticky", "top": "0",
            "zIndex": "1000", "width": "100%", "boxSizing": "border-box"
        },
        children=[brand_logo, signin_btn]
    )
...
def build_home_content():
    ...
    return html.Div(style={"padding": "24px 32px", "width": "100%", "maxWidth": "1280px", "margin": "0 auto", "boxSizing": "border-box"}, children=[doc_container])

app = dash.Dash(__name__, external_stylesheets=[dbc.themes.BOOTSTRAP], suppress_callback_exceptions=True)
app.title = "Kenvue Integrated Business Planning"
app.layout = html.Div(style={"minHeight": "100vh", "width": "100%", "maxWidth": "100%", "backgroundColor": BG, "fontFamily": "'Inter', 'Kenvue Sans', system-ui, -apple-system, sans-serif", "color": TEXT_DARK, "overflowX": "hidden", "boxSizing": "border-box"}, children=[build_header(), html.Div(id="content", children=build_home_content(), style={"width": "100%", "maxWidth": "100%", "boxSizing": "border-box"})])
