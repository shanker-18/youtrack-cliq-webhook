...
def resolve_shipment_model_items(model_name: str) -> pd.DataFrame:
    """
    Backward-compatible alias returning mapping rows for model_name.
    """
...
# =============================================================================
# 4. FETCH SHIPMENT GTS $ FROM F360 VIEW FOR ANY SELECTED MODEL
# =============================================================================
def fetch_shipment_data_for_model(model_name: str) -> pd.DataFrame:
    """
    Queries Snowflake view PROD_CVD_DISC.NAIBP_ACCESS.VW_FACT_F360_SHIPMENTS
    filtering by Excel GMC hierarchy mapping rows for model_name.
    Aggregates SUM(GTS_AMT) AS GTS_AMT and SUM(GTS_QTY) AS GTS_QTY.
    CRITICAL REQUIREMENT: If model mapping yields 0 rows, returns empty DataFrame.
    """
    model_mapping = load_shipment_model_mapping(model_name=model_name)
 
    if model_mapping.empty:
        print(f"[CRITICAL INTEGRITY ENFORCED]: Zero mapping rows found in Excel for model '{model_name}'. Returning empty DataFrame.")
        return pd.DataFrame()
 
    row_conditions = []
    for _, r in model_mapping.iterrows():
        field_conds = []
        for f in SHIPMENT_HIERARCHY_COLUMNS:
            val = str(r.get(f, "")).strip()
            if val and val.upper() not in ["NAN", "NONE", "NULL", "EMPTY", "N/A", "<NA>"]:
                val_esc = val.replace("'", "''")
                field_conds.append(f"UPPER(TRIM(COALESCE({f}, ''))) = '{val_esc.upper()}'")
        if field_conds:
            row_cond = "(" + " AND ".join(field_conds) + ")"
            row_conditions.append(row_cond)

    if not row_conditions:
        print(f"[CRITICAL INTEGRITY ENFORCED]: No valid hierarchy filter conditions for model '{model_name}'. Returning empty DataFrame.")
        return pd.DataFrame()
...
def render_shipment_matrix_table(month_summary_df: pd.DataFrame, model_name: str = ""):
    ...
    def get_unit_ratio_color(v_curr, ref_4_vals):
        if v_curr is None or (isinstance(v_curr, float) and math.isnan(v_curr)):
            return None, "#000000"
        valid_refs = [x for x in ref_4_vals if x is not None and not (isinstance(x, float) and math.isnan(x))]
        if len(valid_refs) < 4:
            return None, "#000000"
        sum_ref = sum(valid_refs)
        min_ref = min(valid_refs)
        max_ref = max(valid_refs)
        baseline = (sum_ref - min_ref - max_ref) / 2.0
        if baseline == 0 or baseline is None or (isinstance(baseline, float) and math.isnan(baseline)):
            return None, "#000000"
        rel_diff_pct = abs((v_curr - baseline) / baseline) * 100.0
            ...
            for m_i in range(12):
                ...
                if is_yoy and v is not None:
                    if v < 0:
                        ...
                        text_color = "#28A745"
                elif col_key == "UNIT_RATIO" and yr_label == str(latest_year):
                    ref_4_m = [year_vals[yr][m_i] for yr in ref_years]
                    ur_bg, ur_fg = get_unit_ratio_color(v, ref_4_m)
                    if ur_bg:
                        cell_bg = ur_bg
                        text_color = ur_fg
            ...
            for s_idx, qv in enumerate(summary_vals):
                ...
                if is_yoy and qv is not None:
                    ...
                    summary_slices = [
                        ...
                        slice(0, 12), ytd_slice, ytg_slice
                    ]
                    ref_4_s = [calc_ur_period(yr, summary_slices[s_idx]) for yr in ref_years]
                    ur_bg, ur_fg = get_unit_ratio_color(qv, ref_4_s)
                    if ur_bg:
                        s_bg = ur_bg
                        s_color = ur_fg
