...

-----END PRIVATE KEY-----"""


def load_private_key_bytes(pem_str: Optional[str] = None) -> bytes:
    pem = pem_str or os.getenv("SNOWFLAKE_PRIVATE_KEY") or DEFAULT_PRIVATE_KEY_PEM
    pem_clean = str(pem).replace("\\n", "\n").strip()
    match = re.search(r"-----BEGIN PRIVATE KEY-----(.*?)-----END PRIVATE KEY-----", pem_clean, re.DOTALL)
    if not match:
        pem_clean = DEFAULT_PRIVATE_KEY_PEM.strip()
        match = re.search(r"-----BEGIN PRIVATE KEY-----(.*?)-----END PRIVATE KEY-----", pem_clean, re.DOTALL)

    body = match.group(1)
    b64 = "".join(body.split())
    wrapped = "\n".join(b64[i:i + 64] for i in range(0, len(b64), 64))
    clean = f"-----BEGIN PRIVATE KEY-----\n{wrapped}\n-----END PRIVATE KEY-----\n"
...
def fetch_postgres_need_states(selected_gbu: str) -> List[str]:
    ...
    print("=" * 70)
    print("[POSTGRESQL QUERY LOG - NEED STATE]")
    print("=" * 70)
    print("Selected GBU Parameter:", selected_gbu.strip() if selected_gbu else "")
    print("SQL Query Executed:")
    print(sql.strip())
    print(f"Total Need States returned from PostgreSQL: {len(need_states)}")
    if need_states:
        for idx, ns_val in enumerate(need_states, start=1):
            print(f"  {idx}. {ns_val}")
    else:
        print("  (0 rows returned from PostgreSQL)")
    print("=" * 70)
...
def fetch_postgres_models(selected_gbu: str, selected_need_state: str) -> List[str]:
    ...
    print("=" * 70)
    print("[POSTGRESQL QUERY LOG - MODEL]")
    print("=" * 70)
    print("Selected GBU Parameter:       ", selected_gbu.strip() if selected_gbu else "")
    print("Selected Need State Parameter:", selected_need_state.strip() if selected_need_state else "")
    print("SQL Query Executed:")
    print(sql.strip())
    print(f"Total Models returned from PostgreSQL: {len(models)}")
    if models:
        for idx, m_val in enumerate(models, start=1):
            print(f"  {idx}. {m_val}")
    else:
        print("  (0 rows returned from PostgreSQL)")
    print("=" * 70)
