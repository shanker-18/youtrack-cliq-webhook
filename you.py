...
HOW_TO_USE_STEPS = [
    ("Select a customer", "from the sidebar filter. All tabs respect this selection."),
    ("Open a tab", "from the sidebar: CDM Input tabs for editable driver forecasts, Data Tabs for source and reference data, and Summary for rolled-up views."),
    ("Choose Brand and Sub-Brand (bag)", "on CDM Input tabs to load the wide-table grid for that slice."),
    ("Review driver rows", "in the Building Block (BB) column. Dollar, percent, and number formatting is applied automatically."),
    ("Click a cell", "to open the Cell Audit modal (see below)."),
    ("Export Data Tabs", "using the Export button next to the Data Tabs section in the sidebar when you need an Excel workbook.")
]

CELL_AUDIT_FEATURES = [
    ("Context panel", "Shows the building block row, fiscal period column, and current cell value."),
    ("New comment", "Add a note explaining your assumption or change. Comments can be saved on any numeric cell, including read-only summary rows."),
    ("Value field", "Appears only on editable driver cells (yellow background, monthly columns). Enter a number or leave blank to clear the value."),
    ("Audit history", "Lists prior comments and value changes for that cell, including timestamp and user session."),
    ("Save", "Writes a comment and/or value change to the audit log. The modal closes on success and the grid refreshes. You must enter a comment or change the value before saving."),
    ("Cancel or close (x)", "Dismisses the modal without saving."),
    ("Commit conflicts", "If another user changed the cell while you had the modal open, Save shows a conflict message. Click Refresh to reload the latest value and history, then try again.")
]

EDITABLE_CELLS_LEGEND = [
    ("Editable driver cell:", "Yellow background on Consumption LE and Inventory LE driver rows. Monthly columns (Jan–Dec) only; quarterly and annual columns are read-only.", "#FEF9C3"),
    ("Read-only summary cell:", "Blue background on summary/total rows and non-driver rows.", "#E0F2FE"),
    ("Not applicable:", "Gray background for non-applicable or excluded intersections.", "#F1F5F9"),
    ("Reference data cell:", "White background for source/reference data tabs.", "#FFFFFF")
]

WHERE_TO_EDIT = [
    ("Consumption LE", "Consumption driver rows (Core Distribution through Base Trends)."),
    ("Inventory LE", "Inventory driver rows (Store Openings through Supply)."),
    ("Shipment LE & Data Tabs", "Comments only; cell values are not editable.")
]

CONSUMPTION_DRIVERS_FULL = [
    ("Core Distribution", "Points of distribution (PODs), expected $PSPW, new items, loss of PODs and deleted item $PSPW, cannibalization from new items, share of search, and search keyword / conversion strategy."),
    ("Discontinuation", "Impact from items removed from the assortment."),
    ("Pricing (Net)", "Price realization and elasticity of pricing actions, including markdown effects on run rates."),
    ("Shelving (Placement + OSA)", "Linear inches, placement quality (e.g. eye level vs. bottom shelf), adjacencies, on-shelf availability (OSA) metrics, and digital traffic changes."),
    ("Merchandising", "Promotional events and their effectiveness (items, off-shelf placement, timing, discount depth and type, competitive impact), display compliance, retail media support, and post-event consumption recovery."),
    ("NPI", "New product introduction pipe and launch assumptions."),
    ("NPI Cannibalization", "Volume shifted from existing items to new products."),
    ("Seasonality", "Forecasted seasonal incidence vs. prior year and customer traffic patterns."),
    ("Base Trends", "Underlying trend not explained by other drivers.")
]

INVENTORY_DRIVERS_FULL = [
    ("Store Openings (Pipe)", "Inventory build associated with new store openings."),
    ("Store Closings (Burn)", "Inventory drawdown from store closures."),
    ("Innovation Pipefill", "Initial inventory placement for new product launches."),
    ("Merchandising Build", "Inventory staged ahead of promotional events and displays."),
    ("Merchandising Bleed", "Inventory drawdown after events. Assumptions should align with display compliance vs. ship and with consumption tab."),
    ("Customer Target Adjust", "Manual adjustments to customer target inventory levels."),
    ("Supply (Fill Rate, Supply Vulnerability)", "Fill rate, allocation, and supply vulnerability effects on inventory.")
]
