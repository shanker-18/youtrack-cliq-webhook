...
def map_date_to_kv_calendar(d_str: str) -> dict:
    ...
    # Dynamic fallback for dates outside explicit Excel calendar dump range (e.g. 2021-2025 dates)
    m_nbr = parsed_dt.month
    m_idx = m_nbr - 1
    y_str = str(parsed_dt.year)
    qtr = f"Q{(m_idx // 3) + 1}"
    m_name = MONTHS[m_idx] if 0 <= m_idx < 12 else "JAN"
    ...
    return {
        ...
        "kv_week_beginning": parsed_dt.strftime('%Y-%m-%d'),
        "kv_week_ending": parsed_dt.strftime('%Y-%m-%d')
    }
